...
public class Door  extends Bloc{
	...
	public Door(Wall wall, int position, int length, boolean open) {
		super((int)(wall.getPosX()+(Math.cos(wall.angle.getRadian())*position)), (int)(wall.getPosY()+(Math.sin(wall.angle.getRadian())*position)), length, wall.width,wall.angle, true);
		this.id = doors.size();
		this.wall = wall;
		wall.setDoor(this);
